        ...
        while (escolha != (-1)) {
            ...
            switch (escolha) {
                case 1:
                    ...
                    System.out
                            .println(passageiro.getName() + " define um destino (" + passageiro.getDestination() + ")");
                ...
                case 2:
                    ...
                    if (passageiros.size() > 0) {
                        ...
                        System.out.println(
                                "Selecionado o passageiro de nome: " + passageiros.get(posPassageiro).getName());
                ...
                case 3:
                    ...
                    if (passageiros.size() > 0) {
                        ...
                        System.out.println("O destino do passageiro " + passageiros.get(posPassageiro).getName()
                                + " se tornou " + passageiros.get(posPassageiro).getDestination());
                    } else {
                        System.out.println("Adicione um passageiro");
                    }
                    break;
                    ...
                    if (passageiros.size() > 0) {
                        if (taxis.size() > 0) {
                            for (posicao = 0; posicao < taxis.size(); posicao++) {
                                if (taxis.get(posicao).getPassageiro() == passageiros.get(posPassageiro)) {
                                    System.out.println(
                                            taxis.get(posicao).getPassageiro().getName() + " ja esta em um taxi");
                                    isLivre = 1;
                                    break;
                                }
                                if (!taxis.get(posicao).isHailed()) {
                                    ...
                                    System.out.println("Taxista " + taxis.get(posicao).getTaxista().getName()
                                            + " esta dirigindo ate o destino ("
                                            + taxis.get(posicao).getPassageiro().getDestination() + ")");
                                    isLivre = 1;
                                    break;
                                }
                            }
                            if (isLivre == 0) {
                                System.out
                                        .println("Todos os taxis estao em uso. Tente outra hora ou adicione um novo.");
                            }
                            isLivre = 0;
                        } else {
                ...
                case 6:
                    if (passageiros.size() != 0) {
                        if (taxis.size() != 0) {
                            ...
                            if (isLivre == 0) {
                                System.out.println(
                                        "O passageiro " + passageiros.get(posPassageiro).getName() + " esta esperando");
                            }
                            isLivre = 0;
                        } else {
                            System.out.println("Adicione um taxi e chame um taxi para o passageiro para "
                                    + passageiros.get(posPassageiro));
                        }
                    } else {
                            ...
                            for (posicao = 0; posicao < taxis.size(); posicao++) {
                                if (taxis.get(posicao).getPassageiro() == passageiros.get(posPassageiro)) {
                                    if (taxis.get(posicao).chegouDestino()) {
                                        System.out.println(
                                                taxis.get(posicao).getPassageiro().getName() + " chegou ao destino");
                                        System.out.println(taxis.get(posicao).getPassageiro().getName()
                                                + " paga o taxista (sem gorjeta)");
                                        System.out.println(taxis.get(posicao).getPassageiro().getName()
                                                + " sai do taxi " + posicao);
                                        taxis.get(posicao).setPassageiro(null);
                                        break;
                                    } else {
                                        System.out.println("Horario do destino de "
                                                + taxis.get(posicao).getPassageiro().getName() + " passou em 1 hora.");
                                        System.out.println("Restam " + (5 - taxis.get(posicao).getHoras())
                                                + "h para chegar ao destino");
